...
contract Zapper is Ownable, IZapper, Pausable, ContractWhitelisted {
	...
	constructor(
		...
		//approve to breakLP
		IERC20(_EMPETH).safeApprove(address(ROUTER), type(uint256).max);
		IERC20(_ESHAREBNB).safeApprove(address(ROUTER), type(uint256).max);
	...
	function zapTokenToLP(
		...
		// ESHARE, EMP, ETH to create ESHARE-ETH or EMP-ETH will hit this if
		if ((_from == token0 || _from == token1) && token0 != WBNB && token1 != WBNB) {
			// Swap half amount for other
			_swap(_from, amount.div(2), _from == token0 ? token1 : token0, address(this), slippageBp);
			...
			ROUTER.addLiquidity(
				...
				IERC20(token0).balanceOf(address(this)).mul(SafeMath.sub(10000, slippageBp)).div(10000),
				IERC20(token1).balanceOf(address(this)).mul(SafeMath.sub(10000, slippageBp)).div(10000),
				msg.sender,
				block.timestamp + 600
			);
		} else {
	...
	function _swapBNBToLP(
		...
		if (token0 != WBNB && token1 != WBNB) {			
			...
			ROUTER.addLiquidityETH{value: swapValue}(
				...
				IERC20(other).balanceOf(address(this)).mul(SafeMath.sub(10000, slippageBp)).div(10000), 
				swapValue.mul(SafeMath.sub(10000, slippageBp)).div(10000), 
				receiver, 
				block.timestamp + 600
			);
		}
